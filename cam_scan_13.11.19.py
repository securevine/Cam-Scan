#Cam-scan for all ports with manual credentials1.json file
#!/usr/bin/python
import os
import sys
import socket
...
def camerascan(url):		
	...
	print "------------------------------------------------"
	c = os.popen("sudo docker run -t -v /media/peter/data/Target/Cameras/Specific/SA/cam_scan/2019-10-14/F/:/tmp/ ullaakut/cameradar -c /tmp/credentials1.json -t %s -p %s " %(str(ip), port)).read()
	print c
	time.sleep(2)
